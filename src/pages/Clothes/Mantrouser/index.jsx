...
const Trouser = () => {
    const [mantrouser, setMantrouser] = useState([]);
    ...
    useEffect(() => {
        axios.get("http://localhost:3004/mantrouser")
            .then((res) => setMantrouser(res.data))
            .catch((error) => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this user?')) {
            await axios.delete(`http://localhost:3004/mantrouser/${id}`);
            alert('User delete successfully');

            // tải lại trang home khi xóa xong
            setMantrouser((preState) => preState.filter((item) => item.id !== id));
        };
    }
